...
template<typename T>
extern inline T				random(T min = 0, T max = 1) { return (max - min) * (rand() / (RAND_MAX + 1.0)) + min; }

template<typename T>
extern inline int			rounded(T val) { int integral = (int)val; return (val - integral < 0.5) ? integral : integral + 1; };
...
extern vector<string> ToStringList(string s, char delim)
{
	vector<string> s_list;
